...
void CharDebug::ctrlPlusClick() // click upleft or upright black dot
{
    ...
    //preProcessor.findingChar = true;
    //DataBundle db =  ImagePreProcessor::extractChar(m_image,QPoint(imageLabel->point.x(),imageLabel->point.y()));
...
{
    //ImagePreProcessor::setBrailleProperty(m_distBetDot,m_distBetCh,m_minDotWidth, m_maxDotWidth,m_errDot,m_errCh,true,true);
    //DataBundle db =  ImagePreProcessor::getBrailleChPosCenter(m_image,QPoint(imageLabel->point.x(),imageLabel->point.y()));
    //imageLabel->setPixmap(QPixmap::fromImage(db.image));
}
...
void CharDebug::openBtnHandler()
{
    ...
    if (!SelectedFile.isEmpty())
    {
            ...
            //resetMemberVariable
            //m_image = ImagePreProcessor::mainImageToBinImage(SelectedFile);
